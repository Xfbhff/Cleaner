...
#define SysTickSecPassed(m,n)		((My_GetTimerTicks()-m)>=SysTickOfSeconds(n))//n秒内的timer中断次数,m为主程序中自定义的TimerTicks,n为要判断的秒数
#define SysTickSecArrive(m,n)		((My_GetTimerTicks()-m)==SysTickOfSeconds(n))//n秒内的timer中断次数,m为主程序中自定义的TimerTicks,n为要判断的秒数

#if FLAG_READ_SYSTICK>0
extern bool myReadFlag_tick;//发送数据标志
#endif
#if	FLAG_SEND_SYSTICK>0
extern bool mySendFlag_tick;//读取数据标志
#endif
